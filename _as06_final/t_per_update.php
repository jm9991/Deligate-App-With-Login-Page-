...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE t_person  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $mobile, $password, $title, $fileName,$fileSize,$fileType,$content, $id));
			Database::disconnect();
			header("Location: t_persons.php");
		}
		else { // otherwise, update all fields EXCEPT file fields
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE t_person  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
...
<body>
    <div class="container">
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="t_per_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
				...
				<div class="control-group">
					...
					<div class="controls">
						<select class="form-control" name="title">
							...
							if (0==strcmp($_SESSION['t_person_title'],'Person')) echo '<option selected value="Person" >Person</option>';
				...
				<div class="form-actions">
					...
					<a class="btn" href="t_persons.php">Back</a>
				</div>
